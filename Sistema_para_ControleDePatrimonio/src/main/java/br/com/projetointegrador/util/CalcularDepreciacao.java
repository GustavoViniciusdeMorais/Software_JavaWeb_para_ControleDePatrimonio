...
import br.com.projetointegrador.model.bem.BemDao;

public class CalcularDepreciacao implements Serializable {
	...
	public int calculaPeriodo(Bem b) {
		...
		int periodo = meses - mesesInvalidos;
	...
	public int calcularMeses(Bem b) {
		// b = new BemDao().selectById(1L);
		int qtdMeses = 0;
		String dataAquisicao = b.getDtAquisicao().toString();
		// String dataVenda = b.getDtVenda().toString();
		...
		try {
			data = df.parse(dataAquisicao);// data = df.parse(dataAquisicao);
		} catch (ParseException ex) {

		}
		long diferencaMeses = (dataB.getTime() - data.getTime())
				/ (1000 * 60 * 60 * 24) / 30;
		qtdMeses = (int) diferencaMeses;
		System.out.println("data da venda: " + dataB);
		return qtdMeses;
		// System.out.println("diferenca de meses: " + diferencaMeses);
		// System.out.println("data aq: " + data);
		// System.out.println("data vend: " + dataB);// data.getTime() -
		// dataB.getTime()
	}
	...
	// @Test
	public int calcularMesesDois(Bem b) {
		GregorianCalendar dataGregoria = new GregorianCalendar();
		// b = new BemDao().selectById(5L);
		int anoDeVenda = 0, anoDeAquisicao = 0;
		if (b.getDtAquisicao() != null) {
			dataGregoria.setTime(b.getDtAquisicao());
			// pega dia aquisicao
			anoDeAquisicao = dataGregoria.get(Calendar.YEAR);
		}
		if (b.getDtVenda() != null) {
			dataGregoria.setTime(b.getDtVenda());
			// pega dia venda
			anoDeVenda = dataGregoria.get(Calendar.YEAR);
		} else {
			LocalDate hoje = LocalDate.now();
			String hj = hoje.format(DateTimeFormatter.ofPattern("yyyy-MM-dd"));
			try {
		...
		int diferencaMeses = (anoDeVenda - anoDeAquisicao) * 12;
		// System.out.println("nova dif mes: " + diferencaMeses);
	...
	/*
	 * public Date transformaData(String x) throws ParseException { Date data =
	 * new Date(); data = df.parse(x);// data = df.parse("2016-12-01"); return
	 * data; } String hj =
